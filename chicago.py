...
def _normalize_for_compare(s):
    ...
    # Collapse hyphen + optional space to a single hyphen so that
    # "Socio- Legal" and "Socio-Legal" compare as identical.
    s = re.sub(r"\s*-\s*", "-", s)

    # After that, drop all punctuation.
    s = re.sub(r"[^a-z0-9 ]", "", s)

    # Collapse any double spaces introduced by punctuation removal.
...
def _normalize_title_for_compare(s):
    """
    Aggressive title normalization for Chicago matching.

    Folds:
      - case
      - hyphens and spaces around them ("Socio- Legal" == "Socio-Legal")
      - "CahyaSupena"  -> "cahya supena"  (split CamelCase)
    ...
    # Split glued CamelCase into two tokens: "CahyaSupena" -> "Cahya Supena"
    s = re.sub(r"(?<=[a-z])(?=[A-Z])", " ", s)

    s = s.lower()

    # Collapse hyphen/space combos to a single hyphen, then drop hyphens
    s = re.sub(r"\s*-\s*", "-", s)
    s = s.replace("-", " ")

    # Remove remaining punctuation, keep letters/digits/space
    s = re.sub(r"[^a-z0-9 ]", " ", s)
...
def _is_same_work_from_reference(reference, openalex_title, openalex_authors):
    """
    Conservative fallback used only to un-flag a reference whose DOI we
    have already confirmed resolves.

    Returns True when BOTH conditions hold:
      1. The manuscript's title, normalized aggressively, CONTAINS the
         OpenAlex title (or vice-versa). This tolerates case, hyphen,
         glued-word, and trailing-period differences.
      2. At least ONE surname from the OpenAlex author list appears in
         the manuscript reference. This tolerates the manuscript listing
         the author as "CahyaSupena" while OpenAlex says "Supena".

    Never makes the tool MORE suspicious; only less.
    """
    ...
    if ref_norm and oa_norm:
        # Either direction acceptable: manuscript may add subtitle,
        # OpenAlex may strip subtitle.
        title_ok = (oa_norm in ref_norm) or (ref_norm in oa_norm)

    if not title_ok:
        # Fallback: high token overlap on title words (>= 80%)
    ...
    # Author check: at least one OpenAlex surname must appear in the
    # manuscript reference. Glued CamelCase is already split, so
    # "CahyaSupena" becomes "cahya supena" and matches "supena".
...
def bibliography_has_truncated_author_list(text):
    """
    Detect common author-list truncation markers that should be manually
    checked in a bibliography/reference list.

    Deliberately conservative: these are explicit truncation phrases,
    not substrings inside personal names.
    """
    ...
    patterns = [
        ...
        r"\band\s+others\b",
        r"\bwith\s+others\b",
        r"\bdkk\.?\b",              # Indonesian: dan kawan-kawan
        r"\bdan\s+lain(?:nya)?\b",
        r"\bcs\.?\b",               # common Indonesian editorial shorthand
    ]
...
def normalize_malformed_locator_text(text):
    ...
    if locator_at_end:
        ...
        locator = locator_at_end.group(1).strip()
        locator = re.sub(r"[.,;:]+$", "", locator)
...
def build_local_chicago_reference_correction(reference):
    """
    Deterministic Chicago reference tidy-up with placeholders.

    - Missing authors -> 'author ???'
    - Single page like 'p. 123' -> 'pp. 123-???'
    - Missing DOI / URL -> 'doi: ???'
    """
    ...
    if not re.search(r"10\.\d{4,9}/", ref) and not re.search(r"https?://", ref):
        ...
        notes.append(
            f"Missing DOI/URL — placeholder inserted: {PLACEHOLDER_DOI}"
        )
...
def detect_footnote_number(line):
    ...
    remainder = clean_text(
        " ".join(span["text"] for span in following_spans)
    )
    if not remainder:
        return None
...
def get_footnote_region_lines(page, lines, continuation_mode=False):
    ...
    numbered_starts = [
        line for line in lines if detect_footnote_number(line) is not None
    ]
...
def extract_chicago_footnotes(uploaded_file):
    ...
    for page_index in range(bibliography_page):
        ...
        candidates = get_footnote_region_lines(
            page,
            lines,
            continuation_mode=(page_index > 0),
        )
...
def get_page_blocks(page):
    ...
    for raw_block in page.get_text("blocks"):
        ...
        if not text:
            continue
        if looks_like_running_header_footer(text, y0, y1, page_height):
            continue
        blocks.append({
...
def classify_column(block, page_width):
    ...
    crosses_midpoint = (
        x0 < page_mid * 0.80
        and x1 > page_mid * 1.20
    )
    if crosses_midpoint:
        return "FULL"
    if block_center < page_mid:
        return "LEFT"
    return "RIGHT"


def reconstruct_column(blocks):
    output = []
    for block in blocks:
...
def line_is_header_footer(line, page_height):
    ...
    if in_header_zone:
        ...
        if re.search(r"\bvolume\s+\d+", text, re.I):
            return True
        if re.search(r"\bno\.?\s*\d+", text, re.I) and re.search(
        ...
        if "…" in text or "..." in text:
            return True
        if re.search(r"\b(journal|jurnal)\b", text, re.I) and re.search(
            r"\b(vol|volume|no|number)\b", text, re.I
        ):
            return True
...
def split_references_from_lines(lines):
    ...
    for column in ("LEFT", "RIGHT"):
        ...
        if meaningful:
            start_margin[column] = min(c["x"] for c in meaningful)
        else:
            start_margin[column] = clusters[0]["x"]
    ...
    for line in lines:
        ...
        if base_x is None:
            current.append(text) if current else current.append(text)
            if not current:
                current = [text]
            continue
        ...
        if re.match(r"^(https?://|www\.|doi\s*:)", text, re.I):
...
def footnote_bibliography_score(footnote_text, reference_text):
    def identity_tokens(text):
        ...
        return [
            ...
            if token not in stop and not re.fullmatch(r"\d{4}", token)
        ]
    ...
    early_overlap = len(foot_early & bib_early) / max(
        1, min(len(foot_early), len(bib_early))
    )
    ...
    return min(
        1.0,
        (0.40 * jaccard) + (0.35 * coverage) + (0.25 * early_overlap) + year_bonus,
    )
...
# ============================================================
# BIBLIOGRAPHY STATISTICS (with OpenAlex + placeholders)
# ============================================================
...
def heuristic_source_type(reference):
    ...
    if any(re.search(p, t, re.I) for p in journal_patterns):
...
def build_bibliography_statistics(references, gpt_results, manuscript_year):
    ...
    for i, ref in enumerate(references, start=1):
        ...
        corrected_reference = clean_text(
            ai.get("revised_bibliography_markdown", "")
    ...
    detected_year_rows = [row for row in detail_rows if row["Year"] is not None]
    recent_count = sum(bool(row["Within Last 10 Years"]) for row in detected_year_rows)
    ...
    return detail_rows, composition_rows, {
        ...
        "recent_pct_detected": round(
            recent_count / len(detected_year_rows) * 100, 1
...
# ============================================================
# MARKDOWN → DOCX RUN HELPER (with red placeholders)
# ============================================================
...
def add_markdown_italics_to_paragraph(paragraph, text):
    add_markdown_to_paragraph(paragraph, text)
...
def create_complete_chicago_report(
    ...
    def format_paragraph(paragraph, text, make_red=False, make_bold=False):
        add_markdown_to_paragraph(paragraph, text, make_red=make_red)
    ...
    for row in bibliography_detail:
        ...
        if doi_suspicious:
            ...
            if reasons:
                ...
                r = p_reason.add_run(
                    f"⚠ Possible fabricated reference: {reasons}"
                )
                r.italic = True
                r.bold = True
                r.font.color.rgb = RGBColor(255, 0, 0)
...
# ============================================================
# COMBINED GPT REQUEST — FOOTNOTES + BIBLIOGRAPHY
# ============================================================
...
""" + json.dumps(
    ...
    try:
        response = client.responses.parse(
            model="gpt-4o-mini",
            input=prompt,
            text_format=CombinedChicagoResult,
        )
        ...
        return {
            ...
            "bibliography": [
                {
                    "number": i,
                    "source_type": heuristic_source_type(reference),
                    "year": extract_reference_year(reference),
                    "status": "MANUAL_CHECK",
...
def render():
    st.markdown(
        ...
        div[class*="st-key-green_container"] .stButton button {
            background-color: #349139 !important;
            color: white !important;
            border: none !important;
        }
        div[class*="st-key-green_container"] .stButton button:hover {
            background-color: #2E7D32 !important;
        }
        div[class*="st-key-red_container"] .stButton button {
            background-color: #E03E3E !important;
            color: white !important;
            border: none !important;
        }
        div[class*="st-key-red_container"] .stButton button:hover {
    ...
    uploaded_files = st.file_uploader(
        ...
        help="Upload up to 10 manuscripts in one batch.",
        key="chicago_file_uploader",
    )
    ...
    manuscript_year = st.number_input(
        ...
        value=2026,
        step=1,
        key="chicago_manuscript_year",
    )

    if "chicago_local_results" not in st.session_state:
        st.session_state["chicago_local_results"] = {}

    if "chicago_batch_results" not in st.session_state:
        st.session_state["chicago_batch_results"] = {}

    if uploaded_files:
        if len(uploaded_files) > 10:
            st.error(
                f"You uploaded {len(uploaded_files)} manuscripts. "
                "The maximum batch size is 10 PDFs."
            )
            st.stop()

        # ========================================================
        # STAGE 1 — EXTRACT & REVIEW (local only) — BLUE
        # ========================================================
        with st.container(key="blue_btn_extract_chicago"):
            extract_batch = st.button(
                f"Extract & Review ({len(uploaded_files)} manuscript"
                f"{'s' if len(uploaded_files) != 1 else ''})",
                use_container_width=True,
                key="chicago_extract_button",
            )

        if extract_batch:
            local_results = {}
            progress = st.progress(0, text="Extracting locally...")

            for file_index, uploaded_file in enumerate(uploaded_files, start=1):
                filename = uploaded_file.name
                result_key = f"{file_index}::{filename}"

                try:
                    progress.progress(
                        int(((file_index - 1) / len(uploaded_files)) * 100),
                        text=f"Local extraction {file_index}/{len(uploaded_files)}: {filename}",
                    )

                    uploaded_file.seek(0)
                    footnote_result = extract_chicago_footnotes(uploaded_file)
                    footnotes = footnote_result.get("footnotes", [])

                    footnotes = attach_page_numbers_to_footnotes(
                        footnotes,
                        footnote_result.get("page_candidates", []),
                    )

                    for note in footnotes:
                        note["has_dan_warning"] = (
                            footnote_has_indonesian_author_conjunction(
                                note.get("text", "")
                            )
                        )
                    ...
                    if bibliography_result.get("found"):
                        references = split_references_from_lines(
                            bibliography_result["lines"]
                        )
                        references = [
                            clean_text(ref)
                    ...
                    match_rows = match_footnotes_to_bibliography(
            ...
            progress.progress(100, text="Local extraction complete.")
            progress.empty()

            st.session_state["chicago_local_results"] = local_results
            st.session_state["chicago_batch_results"] = {}
            st.rerun()

        local_results = st.session_state.get("chicago_local_results", {})

        if local_results:
            # ====================================================
            # MANUSCRIPT SELECTOR
            # ====================================================
            selector_keys = list(local_results.keys())
            selected_key = st.selectbox(
                "Select manuscript to review",
                selector_keys,
                format_func=lambda k: local_results[k].get("filename", k),
                key="chicago_extract_selected_manuscript",
            )

            result = local_results[selected_key]
            filename = result["filename"]

            if result.get("error"):
                st.error(f"Local extraction failed: {result['error']}")
            else:
                ...
                # ---- Footnote extract ----
                with st.expander(
                    f"Extracted Footnotes ({len(footnotes)})",
                    expanded=False,
                    key=f"chicago_extracted_footnotes_{selected_key}",
                ):
                    if footnotes:
                        footnote_df = pd.DataFrame([
                            {
                                "No.": note["number"],
                                "Page(s)": ", ".join(
                                    map(str, note.get("pages", []))
                                ),
                                "Extracted Footnote": note["text"],
                            }
                            for note in footnotes
                        ])
                        st.dataframe(
                            ...
                            use_container_width=True,
                            hide_index=True,
                            height=min(320, 38 * (len(footnotes) + 1)),
                        )
                    else:
                        st.warning("No footnotes were detected.")

                # ---- Bibliography extract ----
                with st.expander(
                    f"Extracted Bibliography ({len(references)})",
                    expanded=False,
                    key=f"chicago_extracted_bibliography_{selected_key}",
                ):
                    if references:
                        ...
                        st.dataframe(
                            ...
                            use_container_width=True,
                            hide_index=True,
                            height=min(320, 38 * (len(references) + 1)),
                        )
                    else:
                        st.warning("No bibliography entries were detected.")

            # ====================================================
            # STAGE 2 — AUTOMATED PROCESSING (AI)
            # ====================================================
            client = get_openai_client()

            if client is None:
                st.warning(
                    "API key is not configured. Local extraction "
                    "above remains available."
                )

            with st.container(key="green_btn_ai_chicago"):
                run_ai = st.button(
                    "Automated Processing Check",
                    use_container_width=True,
                    key="chicago_run_ai_button",
                )

            if run_ai:
                batch_results = {}
                overall = st.progress(
                    0, text="Starting Automated AI check..."
                )

                for file_index, (result_key, local) in enumerate(
                    local_results.items(), start=1
                ):
                    filename = local["filename"]

                    if local.get("error"):
                        batch_results[result_key] = dict(local)
                        continue

                    try:
                        overall.progress(
                            int(((file_index - 1) / len(local_results)) * 100),
                            text=f"AI checking {file_index}/{len(local_results)}: {filename}",
                        )

                        footnotes = local["footnotes"]
                        references = local["references"]
                        match_rows = local["match_rows"]

                        combined_results = check_all_chicago_with_gpt(
                            footnotes, references, client
                        )

                        ai_notes = combined_results.get("footnotes", [])
                        ai_by_no = {
                            int(x["number"]): x
                            for x in ai_notes
                        ...
                        for note in footnotes:
                            ...
                            status = str(
                                ai.get("status", "MANUAL_CHECK")
                            ).upper().strip()
                            if status not in {"OK", "REVISED", "MANUAL_CHECK"}:
                                status = "MANUAL_CHECK"

                            checked = dict(note)
                            checked["ai_status"] = status
                            checked["ai_revised_footnote_markdown"] = ai.get(
                                "revised_footnote_markdown", ""
                            )
                            checked["ai_explanation"] = ai.get("explanation", "")
                            checked["decision"] = (
                                "✓ OK" if status == "OK"
                                else "⚠ REVISED" if status == "REVISED"
                                else "⚠ MANUAL CHECK"
                            )
                            checked_notes.append(checked)

                        bibliography_results = combined_results.get(
                            "bibliography", []
                        )

                        detail_rows, composition_rows, recency_stats = (
                            build_bibliography_statistics(
                                references,
                        ...
                        matched_bibliography_numbers = (
                            get_matched_bibliography_numbers(match_rows)
                        )
                        duplicate_doi_numbers = (
                            get_duplicate_doi_reference_numbers(references)
                        )
                        ...
                        for row in detail_rows:
                            ...
                            cited_in_footnotes = (
                                reference_no in matched_bibliography_numbers
                            )
                            ...
                            final_corrected_reference = clean_text(
                                row["GPT Revised"]
                            ) or original_reference
                            ...
                            final_year = extract_reference_year(
                                final_corrected_reference
                            )
                            if final_year is not None:
                                row["Year"] = final_year

                            truncated_authors = (
                                bibliography_has_truncated_author_list(
                                    original_reference
                                )
                                or bibliography_has_truncated_author_list(
                                    row["GPT Revised"]
                                )
                            )
                            ...
                            chicago_status = str(
                                row.get("Chicago Status", "NOT CHECKED")
                            ).upper().strip()
                            ...
                            withheld = (
                                bool(row.get("DOI Suspicious"))
                                and not bool(row.get("DOI Rescued"))
                            )
                            ...
                            corrected_bibliography_rows.append({
                                ...
                                "Placeholders": ", ".join(
                                    k for k, v in (row.get("Placeholders") or {}).items()
                                    if v
                                ) or "—",
                                "Status": display_status,
                                "DOI Checked": (
                                    "YES" if row.get("DOI Verified") else "NO"
                                ),
                                "DOI Suspicious": (
                                    "⚠️ YES" if withheld else "—"
                                ),
                                "OpenAlex Title": (
                                    row.get("OpenAlex Title") or ""
                                )[:60],
                                "DOI Issues": row.get("DOI Verification Reasons", ""),
                                "Footnote in Bibliography": (
                                    "☑ Checked"
                                    if cited_in_footnotes
                                    else "☐ Unchecked"
                                ),
                                ...
                                "Duplicate DOI": (
                                    "⚠ Yes" if duplicate_doi else "No"
                                ),
                                "Incomplete Author List": (
                                    "⚠ Yes" if truncated_authors else "No"
                                ),
                            })

                        total_references = len(detail_rows)
                        final_source_counts = Counter(
                            row.get("Source Type", "Other") for row in detail_rows
                        )
                        composition_rows = [
                            {
                                "Source Type": source_type,
                                "Count": count,
                                "Percentage": round(
                                    count / total_references * 100, 1
                                ) if total_references else 0.0,
                            }
                            for source_type, count in sorted(
                                final_source_counts.items(),
                        ...
                        recent_count = sum(
                            bool(row.get("Within Last 10 Years"))
                            for row in detected_year_rows
                        )
                        ...
                        recency_stats = {
                            ...
                            "cutoff": cutoff,
                            "recent_count": recent_count,
                            "recent_pct_all": round(
                                recent_count / total_references * 100, 1
                            ) if total_references else 0.0,
                            "recent_pct_detected": round(
                                recent_count / len(detected_year_rows) * 100, 1
                            ) if detected_year_rows else 0.0,
                            "unknown_year_count": (
                                total_references - len(detected_year_rows)
                            ),
                        }
                        ...
                        batch_results[result_key] = {
                            ...
                            "footnotes": footnotes,
                            "references": references,
                            "missing_rows": local["missing_rows"],
                            "match_rows": match_rows,
                            "checked_notes": checked_notes,
                ...
                overall.progress(100, text="Chicago AI check complete.")
                overall.empty()

                st.session_state["chicago_batch_results"] = batch_results
                st.rerun()

            # ====================================================
            # SHOW AI RESULTS
            # ====================================================
            batch_results = st.session_state.get("chicago_batch_results", {})

            if batch_results:
                ai_selector_keys = list(batch_results.keys())
                selected_ai_key = st.selectbox(
                    "Select manuscript to review",
                    ai_selector_keys,
                    format_func=lambda k: batch_results[k].get("filename", k),
                    key="chicago_ai_selected_manuscript",
                )

                result = batch_results[selected_ai_key]
                filename = result.get("filename", "Unknown PDF")

                if result.get("error"):
                    st.error(f"AI processing failed: {result['error']}")
                else:
                    checked_notes = result.get("checked_notes", [])
                    recency_stats = result["recency_stats"]
                    missing_rows = result["missing_rows"]
                    match_rows = result["match_rows"]
                    composition_rows = result["composition_rows"]

                    source_lookup = {
                        row["Source Type"]: f"{row['Count']} ({row['Percentage']}%)"
                        for row in composition_rows
                    }

                    overview_col, composition_col = st.columns(2, gap="large")

                    with overview_col:
                        citation_count = len(result.get("footnotes", []))
                        citation_requirement = (
                            f"Yes ({citation_count})" if citation_count > 15
                            else f"No ({citation_count})"
                        )

                        detail_rows = result.get("detail_rows", [])
                        doi_checked = sum(
                            1 for r in detail_rows if r.get("DOI Verified")
                        )
                        doi_suspicious = sum(
                            1 for r in detail_rows if r.get("DOI Suspicious")
                        )
                        doi_suspicious_pct = (
                            doi_suspicious / recency_stats["total"] * 100
                            if recency_stats["total"] else 0.0
                        )

                        overview_df = pd.DataFrame([
                            {
                                "Metric": "Total References",
                                "Value": str(recency_stats["total"]),
                            },
                            {
                                "Metric": "Citations > 15",
                                "Value": citation_requirement,
                            },
                            {
                                "Metric": "% Last 10 Years",
                                "Value": f"{recency_stats['recent_pct_all']}%",
                            },
                            {
                                "Metric": "Footnotes Missing from Bibliography",
                                "Value": str(len(missing_rows)),
                            },
                            {
                        ...
                        composition_df = pd.DataFrame([
                            {
                                "Source Type": source_type,
                                "Count / Percentage": (
                                    source_lookup.get(source_type, "0 (0.0%)")
                                ),
                            }
                            for source_type in [
                                "Journal Article",
                    ...
                    show_footnote_comparison = st.toggle(
                        "Footnote Comparison",
                        value=False,
                        key=f"chicago_show_footnote_comparison_{selected_ai_key}",
                    )

                    if show_footnote_comparison:
                        footnote_comparison_rows = []
                        for note in checked_notes:
                            raw_status = str(
                                note.get("ai_status", "MANUAL_CHECK")
                            ).upper().strip()
                            display_status = (
                            ...
                            original_note = clean_text(note.get("text", ""))
                            corrected_note = clean_text(
                                note.get("ai_revised_footnote_markdown", "")
                            ) or original_note

                            footnote_comparison_rows.append({
                                "No.": note.get("number", ""),
                                "Original Footnote": original_note,
                                "Corrected AI Version": corrected_note,
                                "Status": display_status,
                            })

                        if footnote_comparison_rows:
                            st.dataframe(
                                pd.DataFrame(footnote_comparison_rows),
                                use_container_width=True,
                                hide_index=True,
                                height=230,
                            )
                    ...
                    show_bibliography_comparison = st.toggle(
                        "Bibliography Comparison",
                        value=False,
                        key=f"chicago_show_bibliography_comparison_{selected_ai_key}",
                    )

                    if show_bibliography_comparison:
                        corrected_rows = result.get("corrected_rows", [])
                        extracted_references = result.get("references", [])

                        if corrected_rows:
                            bibliography_comparison_df = pd.DataFrame(
                                corrected_rows
                            )
                            ...
                            existing_preferred = [
                                ...
                                if col in bibliography_comparison_df.columns
                            ]
                            remaining_columns = [
                                col for col in bibliography_comparison_df.columns
                                if col not in existing_preferred
                            ]

                            bibliography_comparison_df = bibliography_comparison_df[
                                existing_preferred + remaining_columns
                            ]

                            st.caption(
                                f"Showing {len(bibliography_comparison_df)} of "
                                f"{len(extracted_references)} extracted "
                                "bibliography entries."
                            )

                            if len(bibliography_comparison_df) != len(extracted_references):
                                st.error(
                                    "Reference-count mismatch: "
                                    f"{len(extracted_references)} references extracted, "
                                    f"{len(bibliography_comparison_df)} comparison rows produced."
                                )

                            st.dataframe(
                                bibliography_comparison_df,
                                use_container_width=True,
                                hide_index=True,
                                height=280,
                            )
                    ...
                    if report_docx:
                        ...
                        st.download_button(
                            ...
                            use_container_width=True,
                            key=f"chicago_download_{selected_ai_key}",
                        )